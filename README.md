...
├── index_ko.html           ← Landing page (KO)
├── apac/
│   ├── index.html          ← "Why Now, Why Living?" (EN)
│   └── index_ko.html       ← "Why Now, Why Living?" (KO)
└── seoul/
    ├── index.html          ← "Demand, Supply, Capital" (EN)
    └── index_ko.html       ← "Demand, Supply, Capital" (KO)
```

| URL | Article | Language |
|---|---|---|
| `/` | Landing — Article Index | EN |
| `/index_ko.html` | Landing — Article Index | KO |
| `/apac/` | Why Now, Why Living? | EN |
| `/apac/index_ko.html` | Why Now, Why Living? | KO |
| `/seoul/` | Demand, Supply, Capital | EN |
| `/seoul/index_ko.html` | Demand, Supply, Capital | KO |
...
*Forthcoming:*
- Essay 2 · Macro and Policy — Where rates, household debt, and regulation meet
...
2. Add `index.html` (English) and `index_ko.html` (Korean) using the same design system
3. Copy `Remarble-Primary.png` and `Remarble-White.png` into the folder
4. Update both landing pages (`/index.html` and `/index_ko.html`) — add a new `<a class="article-card">` block in the `.article-grid` section
5. Commit and push
...
| Body serif | Newsreader (EN) / Noto Serif KR (KO) |
| Sans-serif | Inter |
| Mono | JetBrains Mono |
